...
class FileMaker_Parser_FMResultSet
{
    ...
    function parse($V0f635d0e)
    {
        ...
        xml_set_element_handler($this->xml, '_start', '_end');
        xml_set_character_data_handler($this->xml, '_cdata');
        if ( ! @xml_parse($this->xml, $V0f635d0e)) {
        ...
        if (version_compare($this->Vf5bf48aa['version'], FileMaker::getMinServerVersion(), '<')) {
            return new FileMaker_Error($this->_fm,
                'This API requires at least version ' . FileMaker::getMinServerVersion() . ' of FileMaker Server to run (detected ' . $this->Vf5bf48aa['version'] . ').');
        }
        $this->V6de51026 = true;
    ...
    function setResult($Vb4a88417, $V561b2299 = 'FileMaker_Record')
    {
        ...
        $V6e52c40b = array();
        foreach ($this->databases as $Vde17f0f2) {
